...
* Base-pair probabilities P(i, j) from inside x outside / Z_total
* Visualize as dot plot
* Validate: Sigma_structures e^{-E/kT} matches brute-force enumeration on
  n <= 10
...
Algorithms:
* **Inside** alpha[i, j, N] = P(N =>* x_{i:j+1})
* **Outside** beta[i, j, N] = P(S =>* x_{1:i} N x_{j+1:n+1})
...
* Golden tests: our MFE within eps of ViennaRNA's on small sequences
