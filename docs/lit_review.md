...
Given an RNA primary sequence x = x_1 ... x_n in {A,C,G,U}^n, predict its
...
(O(n^3) time, O(n^2) space). Define

  gamma(i, j) = max |S| over nested matchings on x_i ... x_j.

Recurrence (Nussinov):
  gamma(i, j) = max{ gamma(i+1, j),                        # i unpaired
                gamma(i, j-1),                          # j unpaired
                gamma(i+1, j-1) + delta(x_i, x_j),          # i,j paired
                max_{i<k<j} gamma(i, k) + gamma(k+1, j) }   # bifurcation

with delta = 1 if x_i-x_j is a valid pair, else 0.
...
Moves from pair counts to thermodynamic **free energy** DeltaG. The
...
Complexity: O(n^3) in time (O(n?) if you allow arbitrary internal
...
Replaces Zuker's min with sum-of-exponentials:
  Z(i, j) = Sigma_structures exp(?DeltaG / kT)
...
Yields **base-pair probabilities**:
  P(i, j) = Z_inside(i+1, j-1) * exp(?DeltaG_pair/kT) * Z_outside(i, j) / Z_total
...
Let G = (N, Sigma, R, S) be a (P)SCFG in Chomsky-like form. The three
fundamental problems (analogous to HMMs):

| Problem | HMM | SCFG |
|---------|-----|------|
| Likelihood P(x\|theta) | forward | **Inside** (O(n^3N)) |
| Most likely parse | Viterbi | **CYK** (O(n^3N)) |
| Posterior P(rule\|x) | fwd-bwd | **Inside-Outside** (O(n^3N^2)) |
